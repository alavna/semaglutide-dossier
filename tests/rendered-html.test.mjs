...
test("server-renders the complete Semaglutide dossier snapshot", async () => {
  ...
  assert.match(html, />19<\/[^>]+><[^>]*>Sources</);
  assert.match(html, />22<\/[^>]+><[^>]*>Claims</);
  assert.match(html, />10<\/[^>]+><[^>]*>Perspectives</);
  assert.match(html, />6<\/[^>]+><[^>]*>Contradictions</);
  assert.match(html, />16<\/[^>]+><[^>]*>Evidence gaps</);
